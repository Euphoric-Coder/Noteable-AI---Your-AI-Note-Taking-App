...
export const aiAssist = async (query, answer) => {
  ...
  Instructions:
    ...
    7. But don't use <ul> and <ol> tags but use multiple <p> tags and enclose the whole answer in <p> tags separately and also use bold (using the <strong> tag) and italics to make it look better and also use numbers to define any point in the answer with a line space.
    ...
    `;
    const load = toast.loading("Generating your response.... please wait");
    ...
    if (!formattedResponse || formattedResponse.length < 5) {
      toast.dismiss(load);
      toast.error("AI returned no meaningful response.");
      return "";
    }
